...
class VaDE(nn.Module):
    def __init__(self, input_dim=784, latent_dim=10, n_centroids=10, binary=False,
        encodeLayer=[500,500,2000], decodeLayer=[2000,500,500], activation="ReLU", dropout=0, is_bn=False):
        super(self.__class__, self).__init__()
        self.z_dim = latent_dim
        self.n_centroids = n_centroids
        self.encoder = buildNetwork([input_dim] + encodeLayer, activation=activation, dropout=dropout, is_bn=is_bn)
        self.decoder = buildNetwork([latent_dim] + decodeLayer, activation=activation, dropout=dropout, is_bn=is_bn)
        self._enc_mu = nn.Linear(encodeLayer[-1], latent_dim) # why linear no activation?
        self._enc_log_sigma = nn.Linear(encodeLayer[-1], latent_dim)
        ...
        self.create_gmmparam(n_centroids, latent_dim)
