...
class ScriptArguments:
    ...
    layer_normalization: Optional[str] = field(default="LayerNorm") # TODO "BatchNorm" or "LayerNorm" or "none". 
    ...
    use_exponential_moving_average_or_optimum_targets: Optional[str] = field(default="average")
    use_metrics_or_losses_for_lagrange_updates: Optional[str] = field(default="losses")
...
def argument_parser(script_args: ScriptArguments) -> Tuple[ScriptArguments, Dict[str, Any]]:
    ...
    script_args.output_path  = script_args.output_path + script_args.model_name.split("/")[-1]

    script_args.run_name = f"{script_args.dataset.value}_" + script_args.run_name + f"_{datetime.now().strftime('%m%d_%H%M%S')}_epo{script_args.num_train_epochs}_s{script_args.seed}" if script_args.run_name is not None else None
...
def obtain_tokenizer(script_args: ScriptArguments, preset: Dict[str, Any], checkpoint_path=None) -> AutoTokenizer:
    ...
    tokenizer = AutoTokenizer.from_pretrained(script_args.model_name if checkpoint_path is None else checkpoint_path, **tokenizer_kwargs)
